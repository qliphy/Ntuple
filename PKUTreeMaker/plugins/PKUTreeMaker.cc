  ...
  ,effAreaPhotons_((iConfig.getParameter<edm::FileInPath>("effAreaPhoFile")).fullPath() )
{
  ...
  LheToken_=consumes<LHEEventProduct> (iConfig.getParameter<edm::InputTag>( "externalLHEProducer") ) ;
...
//  outTree_->Branch("mtVlepnew"          ,&mtVlepnew         ,"mtVlepnew/D"         );
...
//std::cout<<"ip = "<<photons->size()<<std::endl;  
...
//################Jet Correction##########################
    ...
    sort (jets.begin (), jets.end (), mysortPt);
           for (size_t i=0;i<jets.size();i++) {
             if(iphoton>-1) {    //why iphoton>0 ?? why not iphoton>-1 ??
